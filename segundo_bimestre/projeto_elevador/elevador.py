...
def init_sim():
    # Elevadores
    estado_e1 = dict(
        [('andar_ant', 0), ('andar', 0), ('npass', 0), ('passageiros', []), ('direção', 0), ('chamadas', [])])
    estado_e2 = dict(
        [('andar_ant', 0), ('andar', 0), ('npass', 0), ('passageiros', []), ('direção', 0), ('chamadas', [])])
    # simulação
...
def saidas():
    ...
    descendo = random.randint(0, POPULACAO[andar])  # determina quantos passageiros
    FILAS_DE_ANDAR[andar] += descendo  # Atualiza fila
    atualiza_chamadas(andar)


def loop_de_evento(n):
    global CICLOS, eficiencia
    i = 0
    entregas = []
    while True:
        # gera_fila()
        ...
        if i < n:
            print(
                f"{i}:Tamanho da fila: {sim['fila']}; Embarcados: {EMBARCADOS}; Desembarcados: {DESEMBARCADOS}; Andares Percorridos: {ANDARES_PERCORRIDOS}\r",
                end="")
        else:
            eficiencia = DESEMBARCADOS / ANDARES_PERCORRIDOS
            print(
                f"{i}:Tamanho da fila: {sim['fila']}; Embarcados: {EMBARCADOS}; Desembarcados: {DESEMBARCADOS}; Andares Percorridos: {ANDARES_PERCORRIDOS}; Eficiência: {eficiencia}")

    return entregas, eficiencia
